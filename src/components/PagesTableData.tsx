  ...
  const fetchData = async (keyword: string, pageNumber: number) => {
    try {
      ...
      const response = await cms.get(
        `/admin/page?Keyword=${keyword}&PageSize=${itemsPerPage}&Page=${pageNumber}`
      );
    ...
    },
    {
      ...
      header: ({ column }) => (
        <Button
          ...
          onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
        >
          Sections <ArrowUpDown className="ml-2 h-4 w-4" />
        </Button>
      ),
      cell: ({ row }) => {
                ...
                {sections.map((section, index) => (
                  <li
                    ...
                    className={`p-2 rounded-lg ${
                      section.status === "Active" ? "bg-lamaSky" : "bg-red-200"
                    }`}
                  >
                    {section.type}
                  </li>
