...
export default function SectionTableData() {
  ...
  const fetchData = async (keyword: string, pageNumber: number) => {
    try {
      ...
      const response = await cms.get(
        `/admin/section?Keyword=${keyword}&PageSize=${itemsPerPage}&Page=${pageNumber}`
      );
  ...
  const columns: ColumnDef<any>[] = [
    {
      ...
      header: ({ column }) => (
        ...
        >
          STT <ArrowUpDown className="ml-2 h-4 w-4" />
        </Button>
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-start px-4">{row.getValue("type")}</div>
      ),
    },
    {
      accessorKey: "status",
